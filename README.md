...
- ✅ **138 個 TDD 測試** (137 unit + 1 integration,含 VAD / flash-attn / parallel / priority / batch)
...
| **native ARM64 + KleidiAI** | **`--quant q4_0 -ac 640 -nfa`** | **6.2 s** | **1.07×** 🚀 |
...
```
asr-local <audio> [<audio2> <audio3> ...]   # 可多檔批次
...
- `tests/` — 139 pytest tests (138 unit + 1 slow integration)
...
Clone this repo, run `scripts\setup.ps1` once (installs LLVM + VS ARM64 workload + builds whisper.cpp), then `uv run python -m asr_local.cli your-audio.m4a`. See [Quick start](#quick-start).
...
Windows 11 ARM64 + 16 GB RAM + 5 GB 空間。執行 `scripts\setup.ps1` 安裝 LLVM/VS ARM64/build whisper.cpp (首次約 10 分鐘),之後 `uv run python -m asr_local.cli 音檔.m4a`。
...
Yes. `uv run python -m asr_local.cli a.m4a b.m4a c.wav` — model loads once, amortized across all files. Each gets its own `<name>.transcript.txt`.
